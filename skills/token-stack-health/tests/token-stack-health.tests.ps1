...
if ($result.components.Count -ne 9) { throw "expected 9 rows (harness, model, topology, ponytail, caveman, rtk, headroom, harvester, contextdb), got $($result.components.Count)" }

$topology = $result.components | Where-Object Name -eq 'topology'
if (-not $topology) { throw 'topology component missing' }
...
$harvester = $result.components | Where-Object Name -eq 'harvester'
$contextdb = $result.components | Where-Object Name -eq 'contextdb'
if (-not $harvester -or -not $contextdb) { throw 'harvester or contextdb missing' }
