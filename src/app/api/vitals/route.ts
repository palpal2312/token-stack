...
async function computeVitals() {
  // agy.exe exits instantly on a plain pipe (needs a console), so its probe
  // goes through the PTY lane; every other CLI keeps the pipe lane.
  const spec = cliSpec("antigravity");
  const agyBin = spec ? defaultBinFor(spec) : null;
  const [claude, openclaw, hermes, antigravity] = await Promise.all([
    ...
    agyBin
      ? ptyRun(agyBin, ["--version"], { timeoutMs: 6000 })
      : Promise.resolve({ ok: false, code: null, stdout: "", stderr: "agy.exe not found at the default path.", durationMs: 0 }),
  ]);
