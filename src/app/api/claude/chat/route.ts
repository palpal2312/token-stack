...
import { LineBuffer } from "@/lib/builders/protocol";
...
export async function POST(req: Request) {
  ...
  let body: { prompt?: unknown; cwd?: unknown; ultracode?: unknown; project?: unknown; resumeRunId?: unknown; history: ChatMsg[]; builderId?: unknown };
  try { body = await req.json(); }
  catch { return NextResponse.json({ error: "Body must be JSON." }, { status: 400 }); }
  const { prompt, cwd, ultracode, project, resumeRunId, history, builderId } = body;
  ...
  // An optional Builder profile picks which Claude account answers (a second
  // OAuth login, or an API key). Omitted, this tab runs exactly as it always has.
  let builder: Awaited<ReturnType<typeof chatSpawnOptions>>;
  try { builder = await chatSpawnOptions(builderId, "claude"); }
  catch (e) {
    return new Response(String(e instanceof Error ? e.message : e), { status: 400 });
  }

  const args: string[] = ["-p", "--model", builder.model ?? CLAUDE_MODEL];
  ...
  const child = spawnStream("claude", args, {
    cwd: runCwd,
  ...
  // Line-buffer for run capture (separate from forwarding the raw stream).
  const captureLines = new LineBuffer();
  let lastSave = 0;
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      child.stdout.on("data", (b: Buffer) => {
        ...
        if (run) {
          for (const line of captureLines.push(s)) captureLine(line);
          void maybeSave();
        }
      });
      ...
      child.on("close", async (code) => {
        ...
        if (run) {
          const rest = captureLines.flush();
          if (rest.trim()) captureLine(rest);
