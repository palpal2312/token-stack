...
export async function POST(req: Request) {
  ...
  let body: { keyword?: unknown; transcriptSlug?: unknown; transcriptText?: unknown; slug?: unknown };
  try { body = await req.json(); }
  catch { return NextResponse.json({ error: "Body must be JSON." }, { status: 400 }); }
  const { keyword, transcriptSlug, transcriptText, slug } = body;
  ...
  const slugSafe = slug; // const snapshot: narrowing doesn't reach the hoisted sniff() below
        ...
        if (evt.type === "assistant" && evt.message?.content) {
          for (const part of evt.message.content) {
            if (part.type === "tool_use" && part.name === "Write" && typeof part.input?.file_path === "string") {
              const article = articleFromPath(part.input.file_path, slugSafe.trim());
