...
import { extractHtmlOutput, KanbanExecutorError, runKanbanBuilder } from "@/lib/agent-kanban/executor";
...
export async function POST(req: Request) {
  ...
  const { id, title, brief, goal, mode, profile, siteId } = body;
  ...
  if (engine === undefined) engine = configured.builder.engine;

  if (engine === "builder") {
    const builderId = typeof body.builderId === "string" && body.builderId
      ? body.builderId : configured.builder.builderId;
    if (!builderId) return NextResponse.json({ error: "No verified builder role is configured." }, { status: 409 });
    const prompt = `Card: ${title}\nBuild: ${brief || title}`;
    try {
      const result = await runKanbanBuilder(builderId, "builder", SYS, prompt, { signal: req.signal });
      const html = extractHtmlOutput(result.text);
      const ok = !!html && /<(html|body|canvas|svg|div|style)/i.test(html) && html.length > 120;
      if (!ok) {
        return NextResponse.json({
          ok: false, model: result.model, effort: result.effort, bytes: 0,
          verdict: "rejected", note: "the builder returned no usable HTML artifact",
        });
      }
      const bytes = Buffer.byteLength(html);
      await recordBuild({
        id, title: String(title).slice(0, 80), brief: String(brief ?? "").slice(0, 240),
        goal: String(goal ?? "").slice(0, 160), model: result.model, bytes, createdAt: Date.now(),
      }, html);
      return NextResponse.json({
        ok: true, model: result.model, effort: result.effort, bytes,
        verdict: "approved", note: "real HTML in your workspace — verified",
        engine: "builder", builderId,
      });
    } catch (error) {
      const status = error instanceof KanbanExecutorError ? error.status : 502;
      return NextResponse.json({
        ok: false, model: builderId, bytes: 0, verdict: "error",
  ...
  if (engine === "hermes" && mode === "seo") {
    ...
    try {
      const raw = await hermesOneShot(prof, seoWriterPrompt(title, typeof brief === "string" ? brief : "", site.name, today), 220_000);
