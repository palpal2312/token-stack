...
export async function POST(req: Request) {
  ...
  const { prompt, dangerouslySkipPermissions, history } = body;
  ...
  const args: string[] = ["-p", buildPromptWithHistory(history as ChatMsg[], prompt)];
  if (dangerouslySkipPermissions === true) args.push("--dangerously-skip-permissions");

  // agy.exe refuses to run without a console (a plain pipe exits silently with
  // no output), so this goes through a PTY — same reason the health probe and
  // the builder chat lane do. The prompt is already positional-first, which is
  // the only argv order agy parses flags in. The bin resolves through the CLI
  // spec (its resolveBin knows the real install path), not the shared config —
  // config.antigravity is empty on machines where which("agy") finds nothing.
  const spec = cliSpec("antigravity");
  const bin = spec ? defaultBinFor(spec) : null;
  if (!bin) {
    return NextResponse.json({ error: "Antigravity is not installed or not configured. Set AGENTIC_OS_ANTIGRAVITY_BIN or install the CLI." }, { status: 500 });
  }
  const out = await ptyRun(bin, args, { timeoutMs: TIMEOUT_MS });
