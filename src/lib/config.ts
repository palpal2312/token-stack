...
export interface AgenticConfig {
  ...
  cursor: string | null; // Cursor CLI — the binary is named `cursor-agent`
  herdrBin: string | null; // Herdr — terminal workspace manager, powers the Code Space
  ruflo: string | null;
...
// Cursor's CLI installs as `cursor-agent`, usually under ~/.local/bin (POSIX) or
// %LOCALAPPDATA%\Programs\cursor-agent (Windows), neither of which the Next server's
// PATH reliably contains.
function cursorBinGuess(): string | null {
  const local = process.env.LOCALAPPDATA ?? path.join(os.homedir(), "AppData", "Local");
  const guesses = [
    path.join(os.homedir(), ".local", "bin", "cursor-agent"),
    path.join(local, "Programs", "cursor-agent", "cursor-agent.exe"),
...
// Herdr's Windows installer lands in %LOCALAPPDATA%\Programs\Herdr\bin; the
// macOS/Linux installers use ~/.local/bin or Homebrew.
function herdrBinGuess(): string | null {
  const local = process.env.LOCALAPPDATA ?? path.join(os.homedir(), "AppData", "Local");
  const guesses = [
...
function loadFileConfig(): Partial<AgenticConfig> {
  const candidates = [
    ...
    // Turbopack must not treat process.cwd() as "trace the whole project".
    // This keeps the project-local override while scoping NFT tracing.
    path.join(/* turbopackIgnore: true */ process.cwd(), "agentic-os.config.json"),
...
export const config: AgenticConfig = {
  ...
  // Cursor CLI — powers Cursor Builder profiles. Note `which()` runs `command -v`,
  // which never succeeds on Windows; the guess list is what resolves it there.
  cursor: process.env.AGENTIC_OS_CURSOR_BIN ?? fileCfg.cursor ?? which("cursor-agent") ?? cursorBinGuess(),
  // Herdr (ogulcancelik/herdr) — the terminal multiplexer behind the Code Space.
  herdrBin: process.env.AGENTIC_OS_HERDR_BIN ?? fileCfg.herdrBin ?? which("herdr") ?? herdrBinGuess(),
