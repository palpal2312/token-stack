...
import { Brain, Search, FileText, Sparkles, Clock, Network, Target } from "lucide-react";
...
type Tab = "search" | "omi" | "recent" | "graph" | "goals";

export default function MemoryPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const tabs: { key: Tab; label: string; count?: number; icon: React.ReactNode }[] = [
    ...
    { key: "goals", label: "Goals & Workflows", icon: <Target size={12} /> },
  ];
  ...
  return (
    <div className={embedded ? undefined : "flex min-h-0 flex-col h-full px-4 md:px-6 py-3"}>
    {!embedded && (
      <header className="flex shrink-0 flex-wrap items-center gap-3 mb-3">
        <PageHeaderIcon gradient="linear-gradient(135deg,#22d3ee,#0284c7)">
          <Brain size={18} />
        </PageHeaderIcon>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[15px] font-semibold text-[var(--cream)]">
            ...
            <HeaderStatPills
              stats={[
                { label: "Obsidian vault", tone: "accent" },
                { label: `${omi.length} omi`, tone: omi.length ? "ok" : "neutral" },
                { label: `${recent.length} recent notes`, tone: recent.length ? "ok" : "neutral" },
              ]}
            />
          </div>
        </div>
        {tab === "graph" && (
          <button
            onClick={() => setGalaxyMode((g) => !g)}
            className="ml-auto inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-[var(--line-soft)] bg-[var(--bg-mid)] px-3 text-[12px] font-medium transition"
    ...
    <Panel
      title={embedded ? "Memory — Obsidian Vault" : undefined}
      accent="system"
      icon={embedded ? <Brain size={14} /> : undefined}
      actions={
        embedded ? (
      ...
      className={embedded ? "min-h-[420px]" : "h-full min-h-0 flex-1 border-0 bg-transparent shadow-none"}
    >
      {tab === "graph" ? (
        <div className="flex flex-col h-full min-h-0">
          {tabBar}
          <div className={`flex-1 min-h-0 rounded-2xl border border-[var(--panel-border)] overflow-hidden relative bg-black ${embedded ? "min-h-[320px]" : ""}`}>
