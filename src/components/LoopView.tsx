  ...
  return (
    <div className="flex min-h-0 flex-col h-full px-4 md:px-6 py-3">
      <header className="flex shrink-0 flex-wrap items-center gap-3 mb-3">
          <PageHeaderIcon gradient="linear-gradient(135deg,#2dd4bf,#0f766e)">
          <RotateCw size={18} />
        </PageHeaderIcon>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[15px] font-semibold text-[var(--cream)]">
            ...
            <span
              ...
              style={{
                borderColor: running ? "rgba(251,191,36,.45)" : "var(--line-soft)",
                color: running ? "#fbbf24" : "var(--cream-mute)",
                background: running ? "rgba(251,191,36,.10)" : "transparent",
              }}
            >
              <span className="h-1.5 w-1.5 rounded-full" style={{ background: running ? "#fbbf24" : "#6e6353" }} />
              {running ? "Running" : "Idle"}
            </span>
            <HeaderStatPills
              stats={[
                { label: `${builds.length} builds`, tone: builds.length ? "accent" : "neutral" },
                { label: `${iters.length} rounds`, tone: iters.length ? "ok" : "neutral" },
              ]}
            />
          </div>
        </div>
