...
/** Video when the clip is actually in the pack, poster + honest note when it
 *  is not — never a <video> pointed at a file that 404s. */
function DemoMedia({ v, available, muted }: {
  v: { src: string; poster: string; title: string };
  available: Set<string> | null;
  muted?: boolean;
}) {
  if (available?.has(v.src)) {
    return (
      <video src={v.src} poster={v.poster || undefined} controls loop playsInline preload="metadata"
             muted={muted} className="block w-full bg-black" style={{ aspectRatio: "16/9", objectFit: "cover" }} />
    );
  }
  return (
    <div className="relative bg-black" style={{ aspectRatio: "16/9" }}>
      {v.poster
        ? <img src={v.poster} alt={v.title} className="block w-full h-full" style={{ objectFit: "cover" }} />
        : null}
      <span className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded text-[10px]"
            style={{ background: "rgba(0,0,0,0.65)", color: "var(--cream-dim)" }}>
        demo clip not in this pack
      </span>
    </div>
  );
}
...
export default function OpenMontageView() {
  ...
  // Ask the pack which demo clips exist before pointing a <video> at any of
  // them — a missing mp4 is a 404 the user can see, so absent clips render as
  // poster cards with a note instead of a dead player.
  useEffect(() => {
    fetch("/api/openmontage/demos", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => setAvailable(new Set(Array.isArray(j.videos) ? j.videos : [])))
  ...
  return (
    <div className="h-full overflow-y-auto sidebar-scroll pr-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {MOVIES.map((v) => (
          <div key={v.src} className="panel overflow-hidden">
            <DemoMedia v={v} available={available} />
            <div className="px-4 py-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {VIDEOS.map((v) => (
          <div key={v.src} className="panel overflow-hidden">
            <DemoMedia v={v} available={available} muted />
            <div className="px-4 py-3">
              <div className="text-sm" style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600, color: "var(--cream)" }}>{v.title}</div>
              <div className="text-[11px] mt-1 leading-snug" style={{ color: "var(--cream-dim)" }}>{v.note}</div>
            </div>
