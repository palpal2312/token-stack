...
//
// `three` is dynamic-imported (Phase 19a U4) so it is its OWN lazy chunk, preloadable on
// intent, never statically bundled into the route. The rAF loop is started only while the
// orb is visible and only when the user has not requested reduced motion (animation-classify).
...
export default function RadarOrb({ image, sweeping = false }: { image: string; sweeping?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let shown = true;
      const visible = elementVisible(shown, true);
      // reduced-motion => render a single static frame, never animate.
      let allowed = classifyAnimation({ visible, reducedMotion, kind: "infinite-rAF" }) === "run";
