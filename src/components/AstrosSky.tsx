...
import { classifyAnimation, elementVisible } from "@/shell/animation-classify";

// Type-only namespace: erased at build (three is NOT statically bundled); the
// runtime value comes from the dynamic `await import("three")` in the effect.
...
//
// `three` is dynamic-imported (Phase 19a U4) so it is its OWN lazy chunk, preloadable
// on intent, never statically bundled. The rAF loop runs only while the sky is visible
// and only when reduced-motion is not requested (animation-classify).
...
export default function AstrosSky({ topics, sweeping, activeIndex, onSelect }: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let shown = true;
      // motion-off under reduced motion; pause while hidden; otherwise run.
      let allowed = classifyAnimation({ visible: elementVisible(shown, true), reducedMotion, kind: "infinite-rAF" }) === "run";
      ...
      const animate = () => {
        ...
        if (hidden || !allowed) { stop(); renderFrame(); return; }
        renderFrame();
      };
      let hidden = false;
      const io = new IntersectionObserver(([e]) => {
        hidden = !e.isIntersecting;
        shown = e.isIntersecting;
        if (hidden) stop();
        else if (allowed && raf === 0) animate();
      }, { root: null });
      io.observe(host);

      if (!allowed) renderFrame();
      else animate();
