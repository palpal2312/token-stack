...
type State = { generation: number; leaseGeneration: number; backendAvailable: boolean; activeCandidate: string | null; approvals: string[]; outbox: string[]; delivered: string[]; lease: Lease | null; durableMutations: number; lastMutation: string | null; rolledBack: boolean; snapshots: Record<string, State>; persistedAt: number };
const clockOffsetArg = process.argv.indexOf("--clock-offset-ms");
const clockOffset = clockOffsetArg >= 0 ? Number(process.argv[clockOffsetArg + 1]) : 0;
if (!Number.isSafeInteger(clockOffset) || Math.abs(clockOffset) > 86_400_000) throw new Error("clock offset must be a safe integer within one day.");
const now = () => Date.now() + clockOffset; const clone = <T>(value: T): T => JSON.parse(JSON.stringify(value)) as T; const markerName = ".s10-live-runtime-owner";
...
function initialState(): State { return { generation: 0, leaseGeneration: 0, backendAvailable: true, activeCandidate: null, approvals: [], outbox: [], delivered: [], lease: null, durableMutations: 0, lastMutation: null, rolledBack: false, snapshots: {}, persistedAt: now() }; }
function isState(value: unknown): value is State { if (typeof value !== "object" || value === null) return false; const s = value as Record<string, unknown>; return Number.isSafeInteger(s.generation) && Number.isSafeInteger(s.leaseGeneration) && typeof s.backendAvailable === "boolean" && (typeof s.activeCandidate === "string" || s.activeCandidate === null) && Array.isArray(s.approvals) && Array.isArray(s.outbox) && Array.isArray(s.delivered) && (s.lease === null || (typeof s.lease === "object" && s.lease !== null && typeof (s.lease as Lease).owner === "string" && Number.isFinite((s.lease as Lease).expiresAt) && Number.isSafeInteger((s.lease as Lease).fencingToken))) && Number.isSafeInteger(s.durableMutations) && (typeof s.lastMutation === "string" || s.lastMutation === null) && typeof s.rolledBack === "boolean" && typeof s.snapshots === "object" && s.snapshots !== null && Number.isFinite(s.persistedAt); }
...
const server = createServer(async (req, res) => { try {
  ...
  if (req.method === "GET" && req.url === "/status") return response(res, 200, { generation: state.generation, persistedAt: state.persistedAt, backendAvailable: state.backendAvailable, activeCandidate: state.activeCandidate, delivered: state.delivered.length, lease: state.lease, leaseGeneration: state.leaseGeneration, durableMutations: state.durableMutations, lastMutation: state.lastMutation, rolledBack: state.rolledBack });
  ...
  if (action === "canary") { const candidate = requireText(input.candidate, "candidate"); const observations = input.observations; if (!state.approvals.includes(candidate)) return response(res, 409, { error: "approval-required", failClosed: true }); if (!Array.isArray(observations) || observations.length < 1 || observations.length > 5) throw new Error("observations must be bounded"); const invalid = observations.some((x) => { const errorRate = typeof x === "object" && x !== null ? (x as Record<string, unknown>).errorRate : undefined; return typeof errorRate !== "number" || !Number.isFinite(errorRate) || errorRate < 0 || errorRate > 1; }); const breached = !invalid && observations.some((x) => (x as Record<string, number>).errorRate > .05); state.activeCandidate = invalid || breached ? null : candidate; state.rolledBack = invalid || breached; save(state); return response(res, 200, { outcome: invalid ? "rolled-back-invalid-observation" : breached ? "rolled-back" : "canary-passed-advisory", publication: "none", observations: observations.length, persistedAt: state.persistedAt, failClosed: invalid || breached }); }
  ...
  if (action === "lease") { const owner = requireText(input.owner, "owner"); const ttlMs = requireTtl(input.ttlMs); const current = state.lease; const acquiredAt = now(); const heldBySameOwner = Boolean(current && current.expiresAt > acquiredAt && current.owner === owner); if (current && current.expiresAt > acquiredAt && current.owner !== owner) return response(res, 409, { error: "lease-held", failClosed: true }); const fencingToken = heldBySameOwner && current!.fencingToken === state.leaseGeneration ? current!.fencingToken : state.leaseGeneration + 1; state.leaseGeneration = Math.max(state.leaseGeneration, fencingToken); state.lease = { owner, expiresAt: acquiredAt + ttlMs, fencingToken }; save(state); return response(res, 200, { lease: "acquired", owner, expiresAt: state.lease.expiresAt, fencingToken, acquiredAt }); }
  if (action === "mutate") { const owner = requireText(input.owner, "owner"); const fencingToken = input.fencingToken; const mutation = requireText(input.mutation, "mutation"); const lease = state.lease; if (!lease || lease.expiresAt <= now() || lease.owner !== owner || !Number.isSafeInteger(fencingToken) || fencingToken !== lease.fencingToken || fencingToken !== state.leaseGeneration) return response(res, 409, { error: "stale-or-invalid-lease", failClosed: true }); state.durableMutations++; state.lastMutation = mutation; save(state); return response(res, 200, { mutation: "accepted", durableMutations: state.durableMutations, persistedAt: state.persistedAt }); }
  ...
  if (action === "restore") { const name = requireText(input.name, "name"); let snapshot: unknown; try { snapshot = state.snapshots[name] ?? JSON.parse(readFileSync(join(root, "snapshots", `${name}.json`), "utf8")); } catch { throw new Error("snapshot is unreadable"); } if (!isState(snapshot)) throw new Error("snapshot is malformed"); const leaseGeneration = state.leaseGeneration; state = clone(snapshot); state.leaseGeneration = Math.max(state.leaseGeneration, leaseGeneration); save(state); return response(res, 200, { restored: name, restoredPersistedAt: (snapshot as State).persistedAt, persistedAt: state.persistedAt }); }
...
export function cleanupS10Runtime(runtimeRoot: string) {
  ...
  if (!runtimeToken) throw new Error("runtime ownership token is unavailable.");
