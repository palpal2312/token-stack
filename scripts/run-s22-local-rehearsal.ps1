  ...
  for ($attempt = 0; $attempt -lt 20; $attempt++) {
    try {
      ...
      if ($health.status -eq 'ok') {
        ...
        if ($listener) { return $process }
      }
    } catch {
      Start-Sleep -Milliseconds 500
    }
  }
  Stop-IsolatedDaemon $process
  throw 'isolated daemon health check failed'
}

function Stop-IsolatedDaemon($process) {
  if ($null -eq $process) { return }
  if (-not $process.HasExited) { Stop-Process -Id $process.Id -Force }
  $process.WaitForExit()
}

function Assert-OwnedListener($process) {
  if ($null -eq $process -or $process.HasExited) { throw 'isolated daemon exited unexpectedly' }
  $listener = Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue |
    Where-Object { $_.OwningProcess -eq $process.Id }
  if (-not $listener) { throw 'isolated daemon does not own the rehearsal port' }
}
...
try {
  ...
  foreach ($process in @($first, $second, $third)) {
    if ($null -ne $process -and -not $process.HasExited) { Stop-IsolatedDaemon $process }
  }
  $env:SEN_PLANE_STORE_DIR = $priorStore
  $env:SEN_PLANE_ADDR = $priorAddress
  if (Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue) { throw 'rehearsal listener remained after cleanup' }
