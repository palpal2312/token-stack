...
`npm run dev` / `npm start` use a custom Node server (`server.ts`) so Code Space can open a duplex WebSocket for the Herdr terminal on the same port. Escape hatches: `npm run dev:next` / `npm run start:next` (no terminal WS).
...
- `http://localhost:3737/claude` — Claude chat (needs Claude Code installed)
- `http://localhost:3737/openclaw` — OpenClaw chat + control room
- `http://localhost:3737/hermes` — Hermes chat + control room
- `http://localhost:3737/memory` — Search your Obsidian vault
- `http://localhost:3737/goals` — Goals (writes to vault)
- `http://localhost:3737/journal` — Daily journal
- `http://localhost:3737/guide` — How-to-build-your-own guide
...
- `http://localhost:3737/guide` once it's running
