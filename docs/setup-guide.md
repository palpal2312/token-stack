...
# Token Stack Setup & Operations Guide (7-Layer Master Stack)

Token Stack integrates a 7-layer context optimization engine controlled by the unified `token-stack` CLI:

| Lifecycle Phase | Layer | Tool | What it cuts / optimizes |
|---|---|---|---|
| **Pre-Coding** | **L0: Code Topology** | `Graphify` / `GitNexus` / `CodeGraph` | Zero-token AST symbol navigation & dependency graph |
| **In-Flight** | **L1: Code Reduction** | `Ponytail` | Enforces standard library, KISS, YAGNI, eliminates boilerplate |
| **In-Flight** | **L2: Word Reduction** | `Caveman` | Strips filler words, enforces concise technical git diffs |
| **In-Flight** | **L3: CLI Output Filter** | `RTK` | Filters verbose terminal & test log output by 60–99% |
| **In-Flight** | **L4: Context Proxy** | `Headroom` | Multi-instance lossless HTTP context compression & prompt cache |
| **Post-Task** | **L5: Knowledge Memory** | `MemoraX Code` | Distills verified task learnings & avoids regression traps |
| **Cross-Session** | **L6: Context Database** | `OpenViking` / `Obsidian` | 3-tier progressive context platform (L0/L1/L2) |

---

## Quickstart with Token-Stack 2.0 CLI

```powershell
# Check live profile and proxy health
token-stack status

# Run 7-layer diagnostic
token-stack doctor

# Launch Headroom daemons
token-stack up --all

# Run 3-stage E2E verification
