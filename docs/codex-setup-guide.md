...
| **Layer 4: API Context** | **Headroom Proxy** | Proxy nén ngữ cảnh và lịch sử hội thoại, duy trì prefix cache | `~/.local/bin/headroom.exe` (Port động: `<free-port>`) |
...
```powershell
$source = "C:\path\to\token-stack\skills"
...
Đồng thời liên kết các skill của `ponytail` và `caveman` vào `$HOME\.codex\skills\`.

---

### Bước 2: Cài đặt & Tích hợp RTK (Layer 3)
1. **Binary chính thức**: `%LOCALAPPDATA%\rtk\rtk.exe`.
2. **Tạo shims hệ thống** (đưa vào thư mục nằm trong `PATH`, ví dụ `~/bin`):
   - `~/bin/rtk` (Git Bash shim: `exec "$LOCALAPPDATA/rtk/rtk.exe" "$@"`)
   - `~/bin/rtk.cmd` (CMD / PowerShell shim: `"%LOCALAPPDATA%\rtk\rtk.exe" %*`)
...
1. **Tạo thư mục dữ liệu SQLite độc lập cho profile**:
   ```powershell
   New-Item -ItemType Directory -Path "$HOME\.codex\headroom-data" -Force | Out-Null
   ```
2. **Dò cổng khả dụng & Cấu hình tệp môi trường `~/.codex/.env` (hoặc `~/.env.codex`)**:
   Hệ thống tự động dò cổng trống trong dải `8787`–`9999`. Gán giá trị cổng tìm được vào `<free-port>`:
   ```dotenv
   HEADROOM_PORT=<free-port>
   HEADROOM_UPSTREAM=<upstream-url>
   HEADROOM_DB_PATH=C:\Users\<username>\.codex\headroom-data\headroom.db
   OPENAI_BASE_URL=http://127.0.0.1:<free-port>/v1
   ANTHROPIC_BASE_URL=http://127.0.0.1:<free-port>
   ```
       ...
       "SessionStart": [
         {
           ...
           "hooks": [
             {
               "type": "command",
               "command": "powershell -NoProfile -ExecutionPolicy Bypass -File \"$HOME\\.codex\\hooks\\headroom-ensure.ps1\"",
               "timeout": 120
             }
           ]
         }
...
- **Hiện tượng**: Nếu cố định cổng cho tất cả các profile (Codex, Claude, Kimi), profile khởi động sau sẽ chiếm dụng hoặc xung đột upstream với profile khởi động trước.
- **Giải pháp**: Thuật toán `Get-NextFreePort` quét toàn bộ dải `8787..9999`, kết hợp kiểm tra cấu hình trong các file `.env.*` và thăm dò socket thực tế (`[System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, $port)`) để tự động gán cổng trống đầu tiên khả dụng.
...
- **Giải pháp**: Bắt buộc thêm tham số `--memory-db-path "$HOME/.<profile>/headroom-data/headroom.db"` riêng biệt cho từng profile.
...
- **Giải pháp**: Nâng cấp `token-stack-health.ps1` và `token-stack-report.ps1` tự động nhận diện tiến trình cha `codex`, tự động đọc model từ `~/.codex/config.toml`, và probe đúng cổng proxy được cấu hình của profile.
...
model    [OK     ] configured=<configured-model> runtime=external
...
headroom [OK     ] installed=true configured=true running=true port=<port> http=200
```
